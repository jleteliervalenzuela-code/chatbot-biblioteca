...
def formato_bibliografia(asig):
    """Arma el bloque de contexto. Para los recursos de catalogo (SD_ILS:NNN)
    sin titulo legible, consulta el Web Service (ILSWS) y agrega el titulo real
    y un dato de disponibilidad. Limitado a MAX_ENRIQUECER consultas en vivo."""
    libros = asig["libros"][:MAX_LIBROS_RESPUESTA]
    total = len(asig["libros"])
    lineas = [f"BIBLIOGRAFIA OFICIAL de la asignatura \"{asig['asignatura']}\""
    ...
    for b in libros:
        titulo = b["titulo"]
        enlace = b["enlace"]
        catkey = ilsws.catkey_desde_url(enlace)
        # Solo consultamos el Web Service si no hay titulo legible y queda cupo
        if catkey and enriquecidos < MAX_ENRIQUECER and titulo in (
            "Recurso del catálogo (físico o digital)", "Recurso digital", "(ver enlace)"):
            ...
            if datos and datos.get("titulo"):
                ...
                if datos.get("autor"):
                    t += f" — {datos['autor']}"
                disp = ilsws.resumen_disponibilidad(datos)
                dig = datos.get("enlace_digital")
                linea = f"- {t} (catkey {catkey}): {enlace}"
                if dig:
                    linea += f" | versión digital: {dig}"
                if disp:
                    linea += f" | {disp}"
                lineas.append(linea)
                continue
        lineas.append(f"- {titulo}: {enlace}")
    if total > MAX_LIBROS_RESPUESTA:
        lineas.append(f"(La asignatura tiene {total} recursos en total; estos son los primeros {MAX_LIBROS_RESPUESTA}. "
                      f"Ofrece al estudiante acotar por tema si necesita mas.)")
    return "\n".join(lineas)
...
# DISPONIBILIDAD DE LIBROS Y VERSIÓN DIGITAL (catálogo en tiempo real)
Cuando entregues un recurso del catálogo que traiga datos del sistema (título real, disponibilidad, versión digital):
- Da el **título** y autor del libro.
- Si el estudiante pregunta si un libro está disponible o quiere ir a buscarlo, PREGÚNTALE primero de qué sede es (en cuál campus estudia o quiere retirarlo). Con eso le dices si hay copias disponibles en SU sede.
- Si el recurso tiene versión digital (enlace incluido en los datos), ofrécela también: así puede leerlo en línea sin ir a la sede.
- Sé honesto con el dato: el sistema indica en qué sedes HAY copias disponibles, no el número exacto por sede. Di "hay copias disponibles en tu sede" o "en este momento no figura disponibilidad en tu sede; sí hay en [otras sedes] o puedes usar la versión digital".
- No inventes disponibilidad ni enlaces: usa solo lo que el sistema te entregue. Si no hay dato de disponibilidad, ofrece la versión digital (si existe) o sugiere consultar con el staff de la biblioteca.
...
- Si quiere la BIBLIOGRAFÍA DEL RAMO: usarás los datos que te entregue el sistema (NO el Descubridor).
   · Si recibes "DATOS PARA ESTA RESPUESTA" con una bibliografía: confirma la asignatura y lista los recursos con título en **negrita** y su enlace, limpio. No inventes títulos ni enlaces. Si un recurso dice "Recurso del catálogo" o "Recurso digital", preséntalo igual con su enlace. Si la lista es muy larga, ofrece acotar por tema.
...
def _contexto_bibliografia(historial):
    ...
    el bloque de contexto a inyectar; si no, None."""
    ultimo = ""
    for x in reversed(historial):
        if x["role"] == "user":
            ...
            break
    tl = ultimo.lower()
    # gatilla si menciona bibliografia/libros de asignatura O si pone un codigo de ramo
