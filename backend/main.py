...
- "LIBRO ENCONTRADO": confirma título y autor, y SIEMPRE pregunta primero: **¿versión digital o copia física?** Luego: si DIGITAL y hay enlace, entrégalo (acceso con credenciales Duoc); si no hay, ofrece el Descubridor. Si FÍSICA, pregunta su sede; cuando responda, el sistema consulta en vivo y le confirmas si está en su sede; si no en su sede pero sí en otra, dile que la solicite como "Préstamo intersede"; si no hay en ninguna, ofrece la digital o el staff.
- Si NO recibes bloque: usa web para identificar el título correcto y ofrece el Descubridor como respaldo. Nunca afirmes que no tenemos un libro sin que el sistema lo haya buscado.
...
def _consulta_disponibilidad_sede(historial):
    ...
    if not catkey:
        ...
        if titulo_libro:
            ...
            mejor = None
            # buscar por TITULO y quedarse con el catkey del titulo mas parecido
            ...
            for d in detalles:
                ...
                if tt == tn or tt == tn_corto:
                    mejor = d
                    break
                if mejor is None and (tn_corto in tt or tt in tn or tn in tt):
                    mejor = d
            # si TITLE no encontro, intentar con GENERAL
            if not mejor:
                detalles = ilsws.buscar_y_detallar(titulo_busqueda, indice="GENERAL", max_resultados=8)
                for d in detalles:
                    tt = _norm_basico(d.get("titulo") or "")
                    if tt == tn or tt == tn_corto or tn_corto in tt:
                        mejor = d
                        break
            if mejor:
...
def _titulo_libro_en_historial(historial):
    ...
    recientes (de los bloques 'LIBRO ENCONTRADO: "..."' o de un titulo en negrita
    que el bot ofrecio). Sirve para dar continuidad cuando el estudiante responde
    'digital', 'fisico' o su sede en un mensaje aparte."""
    for x in reversed(historial):
        if x["role"] != "assistant":
            continue
        c = x["content"]
        m = re.search(r'LIBRO ENCONTRADO:\s*"([^"]+)"', c)
        if m:
            return m.group(1)
        # tambien capturar "tenemos **Titulo**" o '"Titulo" de Autor'
        m2 = re.search(r'tenemos\s+\*?\*?([^*\n.!?]+?)\*?\*?\s+de\s', c, re.IGNORECASE)
        if m2:
...
def _contexto_busqueda_libro(historial):
    ...
    if not libros:
        # Buscar por TITULO (mas preciso que GENERAL, evita ruido como
        # "El oro del rey Midas" al buscar "edipo rey"). Si TITLE no trae nada,
        # caer a GENERAL como respaldo.
        detalles = ilsws.buscar_y_detallar(termino, indice="TITLE", max_resultados=5)
        if not detalles:
            detalles = ilsws.buscar_y_detallar(termino, indice="GENERAL", max_resultados=5)
        # priorizar coincidencia de titulo mas cercana al termino buscado
        tn_busca = _norm_basico(termino)
        def _cercania(d):
            tt = _norm_basico(d.get("titulo") or "")
            if tt == tn_busca:
                return 0
