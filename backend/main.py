...
def _consulta_disponibilidad_sede(historial):
    ...
    if not catkey:
        ...
        if titulo_libro:
            ...
            detalles = ilsws.buscar_y_detallar(titulo_libro, indice="TITLE", max_resultados=5)
            ...
            for d in detalles:
                ...
                if tt == tn:
                    mejor = d
                    break
                if mejor is None and (tn in tt or tt in tn):
                    mejor = d
            if mejor:
...
def _contexto_busqueda_libro(historial):
    ...
    if not cat:
        titulo_busqueda = l.get("titulo") or termino
        keys = ilsws.buscar_catalogo(titulo_busqueda, indice="TITLE", limite=1)
        if keys:
            cat = keys[0]
            l["catkey"] = cat
