...
def _consulta_disponibilidad_sede(historial):
    ...
    if not catkey:
        ...
        if titulo_libro:
            ...
            mejor = None
            # buscar por TITULO, AUTOR y GENERAL (cubre "atlas de netter" = autor)
            vistos = {}
            for indice in ("TITLE", "AUTHOR", "GENERAL"):
                try:
                    for d in ilsws.buscar_y_detallar(titulo_busqueda, indice=indice, max_resultados=6):
                        ck = d.get("catkey")
                        if ck and ck not in vistos:
                            vistos[ck] = d
                except Exception:
                    pass
                if indice == "TITLE" and vistos:
                    # si TITLE ya dio match exacto, preferirlo
            ...
            if not mejor:
                for d in vistos.values():
                    ...
                    if (tt == tn or tt == tn_corto or tn_corto in tt or tt in tn
                            or any(p in au for p in tn_corto.split() if len(p) > 3)):
                        mejor = d
                        break
            if not mejor and vistos:
                mejor = list(vistos.values())[0]
            if mejor:
...
def _contexto_busqueda_libro(historial):
    ...
    # Buscar por TITULO, por AUTOR y por GENERAL, y unir resultados. Esto cubre
    # casos como "atlas de netter" donde "Netter" es el AUTOR, no el titulo
    # (el libro esta catalogado como "Atlas de anatomia humana" de Netter).
    vistos = {}
    for indice in ("TITLE", "AUTHOR", "GENERAL"):
        try:
            for d in ilsws.buscar_y_detallar(termino_busqueda, indice=indice, max_resultados=6):
                ck = d.get("catkey")
                if ck and ck not in vistos:
                    vistos[ck] = d
        except Exception:
            pass
        # si ya tenemos suficientes con TITLE, no hace falta seguir
        if indice == "TITLE" and len(vistos) >= 3:
            break
    detalles = list(vistos.values())
