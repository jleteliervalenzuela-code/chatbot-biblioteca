...
def elegir_modelo(historial):
    ...
    Estrategia de costo: las tareas operativas (buscar libro, bibliografia,
    disponibilidad, horarios, servicios) van a HAIKU, que las hace bien y es
    ~3x mas barato. SONNET se reserva para lo academico de verdad (redactar
    guiado, APA explicado, estructurar informe, analisis)."""
    ultimo = ""
    ...
    # 0. SENALES ACADEMICAS FUERTES -> SONNET (prioridad maxima; son redaccion/ensenanza)
    ...
    # 1. FLUJOS OPERATIVOS -> HAIKU (tienen prioridad; son la mayoria del trafico)
    #    - busqueda de un libro por nombre / disponibilidad
    if _parece_busqueda_libro(ultimo):
        return MODEL_SIMPLE
    #    - respuestas cortas de continuidad (digital/fisico/una sede)
    if len(ultimo.split()) <= 4 and any(p in ultimo for p in (
            "digital", "fisic", "físic", "antonio varas", "maipu", "maipú", "plaza",
            "san ", "puente", "valparaiso", "valparaíso", "vina", "viña", "concepcion",
            "concepción", "alameda", "arauco", "melipilla", "puerto montt", "nacimiento",
            "villarrica", "alonso", "joaquin", "joaquín", "bernardo")):
        return MODEL_SIMPLE
    #    - bibliografia de asignatura (es lookup de datos, no redaccion)
    if any(s in ultimo for s in ("bibliografia", "bibliografía", "libros de", "libro de la asignatura",
                                  "libros de mi", "libro del ramo", "bibliografia de", "que libros")):
        return MODEL_SIMPLE
    #    - servicios y datos simples
    if any(s in ultimo for s in SENALES_SIMPLES):
        return MODEL_SIMPLE

    # 2. TAREAS ACADEMICAS REALES -> SONNET
    if any(s in ultimo for s in SENALES_COMPLEJAS):
        return MODEL_COMPLEJO

    # 3. Mensajes muy cortos y genericos (saludos, "gracias", "ok") -> HAIKU
    if len(ultimo.split()) <= 6:
        return MODEL_SIMPLE

    # 4. Ante la duda (mensajes largos sin senal clara, suelen ser academicos) -> Sonnet
    return MODEL_COMPLEJO
